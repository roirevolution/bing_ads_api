...
require 'ads_common_bing/auth/oauth_handler'
require 'ads_common_bing/auth/oauth2_handler'
...
module AdsCommonBing
  class Api
    ...
    def api_config()
      return AdsCommonBing::ApiConfig
    end
    ...
    #
    # Throws:
    #  - AdsCommonBing::Errors::AuthError or derived if authetication error has
    #    occured
    #
    def authorize(parameters = {}, &block)
      ...
      if token.nil?
        ...
        rescue AdsCommonBing::Errors::OAuthVerificationRequired,
            AdsCommonBing::Errors::OAuth2VerificationRequired => e
          verification_code = (block_given?) ? yield(e.oauth_url) : nil
          # Retry with verification code if one provided.
          if verification_code
            code_symbol =
                e.kind_of?(AdsCommonBing::Errors::OAuthVerificationRequired) ?
    ...
    def validate_service_request(environment, version, service)
      ...
      unless api_config.environment_has_version(environment, version)
        raise AdsCommonBing::Errors::Error,
    ...
    #
    # Args:
    # - auth_handler: instance of an AdsCommonBing::Auth::BaseHandler subclass to
    ...
    def create_auth_handler()
      ...
      return case auth_method
        when :CLIENTLOGIN
          AdsCommonBing::Auth::ClientLoginHandler.new(
              @config,
        ...
        when :OAUTH
          ...
          AdsCommonBing::Auth::OAuthHandler.new(
              @config,
        ...
        when :OAUTH2
          environment = @config.read('service.environment',
              api_config.default_environment())
          AdsCommonBing::Auth::OAuth2Handler.new(
              @config,
              api_config.environment_config(environment, :oauth_scope)
          )
        else
    ...
    def load_config(provided_config = nil)
      @config = (provided_config.nil?) ?
          AdsCommonBing::Config.new(
              File.join(ENV['HOME'], api_config.default_config_filename)) :
          AdsCommonBing::Config.new(provided_config)
      init_config()
    end
