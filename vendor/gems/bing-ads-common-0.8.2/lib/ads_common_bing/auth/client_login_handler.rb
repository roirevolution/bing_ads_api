...
module AdsCommonBing
  module Auth
    ...
    class ClientLoginHandler < AdsCommonBing::Auth::BaseHandler
      ACCOUNT_TYPE = 'GOOGLE'
      ...
      def generate_token(credentials)
        ...
        response = AdsCommonBing::Http.post_response(url, data, @config, headers)
      ...
      def handle_login_error(credentials, response, results)
        ...
        if 'CaptchaRequired'.eql?(results['Error'])
          ...
          raise AdsCommonBing::Errors::CaptchaRequiredError.new(results['Error'],
        ...
        raise AdsCommonBing::Errors::AuthError.new(error_message, error_str,
            results['Info'])
      end
