...
AdsCommonBingForBingAds::Auth::ClientLoginHandler.class_eval do
    ...
    #
    # Raises:
    # - AdsCommonBingForBingAds::Errors::AuthError if validation fails
    #
    def validate_credentials(credentials)
      if credentials.nil?
        raise AdsCommonBingForBingAds::Errors::AuthError, 'No credentials supplied.'
      end

      if credentials[:auth_token].nil?
        if credentials[:user_name].nil?
          raise AdsCommonBingForBingAds::Errors::AuthError,
    ...
    #
    # Raises:
    # - AdsCommonBingForBingAds::Errors::AuthError if authentication fails
    #
    ...
    def generate_token(credentials)
      ...
      response = AdsCommonBingForBingAds::Http.post_response(url, data, @config, headers)
    ...
    def handle_login_error(credentials, response, results)
      ...
      if 'CaptchaRequired'.eql?(results['Error'])
        ...
        raise AdsCommonBingForBingAds::Errors::CaptchaRequiredError.new(results['Error'],
      ...
      raise AdsCommonBingForBingAds::Errors::AuthError.new(error_message, error_str,
          results['Info'])
    end
